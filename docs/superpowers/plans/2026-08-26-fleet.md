...
```
(add `import subprocess` to this file's imports; this test needs `status.resolve_pending_fork_ids` from Task 9 - mark it `@unittest.skip("needs Task 9")` until Task 9 lands, then remove the skip.)
...
- §7 cache model incl. observed TTL, warmth, context, spend, resume-vs-respawn: Tasks 4–5, surfaced in Task 9 ✔; `resume_policy` warning on `wake` — applied in Task 7 `wake()` (non-blocking, as the spec says "warns").
...
- §10 tests 1–7: Tasks 7 (1,2,4), fork test applied in Task 7's live test file (3). The block below is the same code, kept here for the reviewer:
  ```python
  def test_fork_opus_child_gets_brief(self):
      launcher.up("opus")
