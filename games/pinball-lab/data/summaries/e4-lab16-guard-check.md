...
> ⚠ **SECOND STAMP, A DIFFERENT AXIS — RULING: the Stage B `stageBRanked` top-20 claim is WITHDRAWN, INCLUDING HERE, despite this run's own `b` guard passing.** `LAB-20`'s `rankingValidityResult` `topN` fix (landed `1ddd5d8`) reports this run's `b` guard as `ok=true` — but its published `stageBRanked` is the SAME 20 rows as the other two, all at `cpRate` exactly 1 (100% catch, the ceiling). A guard pass on a run whose published top-20 is entirely tied at the maximum means the cut the guard checked fell somewhere else in the population, not that this specific top-20 is a sound ranking (`ledger/handoffs/opus2/20260904T160000Z-gate-refounding-refused-lab20-landed.md` §5): the claim is unsound in all three regardless of what any individual run's guard reports, and withdrawing the other two while leaving this one intact would tell a reader that this one found a real ranking it did not. Every published row is individually TRUE — `cpRate = 1`, ~730 trials each, a real measurement — what's false is "these are the top 20." 101 configurations tie at 100% catch across this population; the 20 shown are whatever the sort happened to emit first, and they span only 3 restAngles, 2 activeAngles, 2 restitutions, 1 injection, 2 policies — reading them as "what wins" is wrong; a reader would be inferring load-bearing parameters from an arbitrary cut. **Take instead**: 101 configurations achieve 100% catch; what they share and what varies freely among them is the real finding here, not this particular 20-row table. `releaseDispersion` is unaffected by this stamp — it already fails and is marked invalid in this file's own body, and was never presented as a ranking.
