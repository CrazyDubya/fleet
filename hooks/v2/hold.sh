...
STALE_S=120
# `objects` and `.t|numbers` keep ONE malformed entry from taking the whole
# list with it. `(now - (.t // 0)) < $max` raised on a non-numeric .t (jq
# exit 5), the `|| echo '[]'` swallowed it, and every pending reply in the
# file - including live ones - silently stopped blocking. Filtering per entry
# fails open only for the entry that is actually broken.
FRESH=$(jq -c --argjson max "$STALE_S" '[.[] | objects | select(((.t|numbers) // 0) > (now - $max))]' "$PEND" 2>/dev/null || echo '[]')
N=$(printf '%s' "$FRESH" | jq 'length' 2>/dev/null || echo 0)
[ "$N" -gt 0 ] || { ledger hold allow none-fresh; exit 0; }
# `// "?"`: a fresh entry missing to/id must still produce a readable message
